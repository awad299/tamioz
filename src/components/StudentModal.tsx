...
const StudentModal: React.FC<Props> = ({
  ...
  const handleChange = (index: number, field: keyof DayData, value: number) => {
          ...
          <table className="student-table">
            ...
            <tbody>
              {localDetails.map((day, idx) => (
                <tr key={idx}>
                  <td>{day.اليوم}</td>
                  {['المشاركة', 'الواجب', 'السلوك', 'الاختبار'].map((field) => (
                    <td key={field}>
                      {isTeacher ? (
                        <input
                          type="number"
                          value={day[field as keyof DayData]}
                          onChange={(e) =>
                            handleChange(
                              idx,
                              field as keyof DayData,
                              parseInt(e.target.value) || 0
                            )
                          }
                        />
                      ) : (
                        <span className={day[field as keyof DayData] === 0 ? 'zero' : ''}>
                          {day[field as keyof DayData]}
                        </span>
                      )}
                    </td>
                  ))}
